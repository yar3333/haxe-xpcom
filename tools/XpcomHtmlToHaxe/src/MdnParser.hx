...
class MdnParser
{
	...
	function parseAttributes(className:String, doc:HtmlDocument) : Array<Attribute>
	{
		...
		{
			var table = attributesH2; do table = table.getNextSiblingElement() while (table != null && table.name != "table");
			if (table != null)
			{
				var rows = table.find(">tbody>tr").slice(1);
				for (row in rows)
				{
					...
					var name = (td[0].findOne(">code") != null ? td[0].findOne(">code") : td[0]).innerHTML.stripTags().htmlUnescape().trim();
					var type = (td[1].findOne(">code") != null ? td[1].findOne(">code") : td[1]).innerHTML.stripTags().htmlUnescape().trim();
	...
	function parseMethods(className:String, doc:HtmlDocument) : Array<Method>
	{
		...
		if (methodsH2 != null)
		{
			var table = methodsH2; do table = table.getNextSiblingElement() while (table != null && table.name != "table");
			if (table != null)
			{
